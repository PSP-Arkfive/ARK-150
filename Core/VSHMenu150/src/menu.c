...
int menu_draw(void) {
    ...
    snprintf(msg, 128, " %s ", g_messages[MSG_ARK_VSH_MENU]);
    ...
    for (i = 0; i < TMENU_MAX; i++) {
        temp = strlen(g_messages[MSG_ADVANCED_VSH + i]);
        if (temp > width)
        	width = temp;
    }
    ...
    for (max_menu = 0; max_menu < TMENU_MAX; max_menu++) {
        ...
        if (g_messages[MSG_ADVANCED_VSH + max_menu]) {
        	...
        	len = strlen(g_messages[MSG_ADVANCED_VSH + max_menu]);
        	...
        	snprintf(msg, 128, " %*s%s%*s ", padding, "", g_messages[MSG_ADVANCED_VSH + max_menu], padding, "");
        	...
        	if (item_str[max_menu]) {
        		...
        		snprintf(msg, 128, "%s", item_str[max_menu]);
...
void button_func(vsh_Menu *vsh) {
    ...
    // copy pad to oldpad
    memcpy(&vsh->buttons.old_pad, &pad, sizeof(SceCtrlData));
}
