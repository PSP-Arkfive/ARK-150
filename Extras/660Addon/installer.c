...
} arkfiles[] =
{
    ...
    { "/vsh/module/ark_recovery150.prx", NULL, 0 },
...
void LoadPkgFiles()
{
    for (int i=0; i<NELEMS(arkfiles); i++){
        if (findPkgFile((void**)&(arkfiles[i].buf), &(arkfiles[i].size), arkfiles[i].path) < 0)
            ErrorExit(5000, "Cannot find file in pkg: %s.", arkfiles[i].path);
    }
}
...
int GetReboot(u8 *dataOut, u8 *dataOut2, int cbExpanded, int decompress)
{
    ...
    if (decompress)
    {
        ...
        if (cbExpanded <= 0)
        {
            ErrorExit(5000, "Cannot decompress reboot.bin (0x%08X)", cbExpanded);
        }    
    }
...
int WriteMsIpl()
{
    ...
    if (cbExpanded <= 0)
    {
        ErrorExit(5000, "Cannot decrypt updater prx: 0x%x.", cbExpanded);
    }
...
void ExtractPrxs(int cbFile)
{
    ...
    if (initres < 0)
    {
        ErrorExit(5000, "pspPSARInit failed!. %d", initres);
    }   
    ...
    while (1)
    {
        ...
        if (res < 0)
        {
            ErrorExit(5000, "PSAR decode error, pos=0x%08X.", pos);
        }
        ...
        if (!strncmp(name, "com:", 4) && comtable_size > 0)
        {
            ...
            {
                ErrorExit(5000, "Error: cannot find path of %s.", name);
            }
        }
        ...
        if (cbExpanded > 0)
        {
            ...
            {        		
                ...
                sprintf(msg, "Writing %s (%d)", name, cbExpanded);
                setInfoMsg(INFO_MSG, msg);
            ...
            else if (!strcmp(name, "02g:00000"))
            {
                ...
                if (_2gtable_size <= 0)
                {
                    ErrorExit(5000, "Cannot decrypt 2g table %08X.", _2gtable_size);
                }
...
{
    setInfoMsg(INFO_MSG, "Creating directories... ");
...
static void CopyRegistry()
{
    ...
    setInfoMsg(INFO_MSG, "Copying registry... ");
...
static void Update()
{
    ...
    setInfoMsg(INFO_MSG, "Reading 1.50 psar... ");
    ...
    setInfoMsg(INFO_MSG, "Extracting and writing 150 prx's to ms");
...
int addon660_install(void)
{
    ...
    if (mod < 0)
    {
        ErrorExit(5000, "Error 0x%08X loading/starting libpsardumper.prx.", mod);
    }

    mod = LoadStartModule("pspdecrypt.prx", PSP_MEMORY_PARTITION_KERNEL);
    if (mod < 0)
    {
        ErrorExit(5000, "Error 0x%08X loading/starting pspdecrypt.prx.", mod);
    }
    ...
    setInfoMsg(INFO_MSG, "Writing custom modules...");
    ...
    NormalExit(5000, "Done! Auto-exiting in 5 seconds.");
